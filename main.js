...
function Func(u, v, k){
	...
	let b = c /2;
	let angle = Math.PI / 2;
	let x0 = c * Math.pow(Math.cos(v), 3);
	let y0 = c * Math.pow(Math.sin(v), 3);
	
	let x = (a + x0 * Math.cos(angle) + y0 * Math.sin(angle)) * Math.cos(u);
	let y = (a + x0 * Math.cos(angle) + y0 * Math.sin(angle)) * Math.sin(u);
	let z = b * u - x0 * Math.sin(angle) + y0 * Math.cos(angle)
	return [x/k, y/k, z/k];
}
...
function getVector1(a, t, k){
	let delta = 0.01;
	let point1 = Func(a, t, k);
	let point2 = Func(a + delta, t, k);
...
function DrawSurface() {
	function DrawFigure(k, getNormal) {
		...
		for (let t = -2*size; t <= 2*size; t += step) {
			let positions = [];	
			let normals = [];
		
			for (let a = -4*size; a <= 4*size; a += step) {
				positions = positions.concat(Func(a, t, k));
				normals = normals.concat(getNormal(a, t, k));
				
				positions = positions.concat(Func(a, t + step, k));
				normals = normals.concat(getNormal(a, t + step, k));
			}	
			drawPrimitive(gl.TRIANGLE_STRIP, [0.9, 0.8, 0.3, 1], positions, normals);
		}
	}
